...
- `Super + K` 快捷鍵面板及功能說明，含截圖選取層（擷取整個螢幕、擷取反白視窗、選取要擷取的視窗）與全螢幕桌面切換
- 網路強制入口網站（網頁驗證入口、登入此網路、有限網際網路存取）與時鐘「按右鍵可切換格式」
- 安裝／移除選單的 Claude 桌面版，並讓 ChatGPT 桌面版、Grok 機器人、語音輸入的移除項與安裝側譯名一致
- Omarchy 更新後的自動重新同步
